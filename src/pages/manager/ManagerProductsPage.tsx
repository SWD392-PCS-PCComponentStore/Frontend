...
const MOCK_PRODUCTS: Product[] = [
  { id: 1, name: 'RTX 4090 Founders Edition', category: 'GPU', price: 45990000, stock: 12, image: '', active: true },
  { id: 2, name: 'Intel Core i9-14900K', category: 'CPU', price: 14990000, stock: 25, image: '', active: true },
  { id: 3, name: 'Samsung 990 Pro 2TB', category: 'Storage', price: 5490000, stock: 40, image: '', active: true },
  { id: 4, name: 'ASUS ROG Maximus Z790', category: 'Motherboard', price: 16990000, stock: 8, image: '', active: true },
  { id: 5, name: 'Corsair Vengeance 32GB DDR5', category: 'RAM', price: 3290000, stock: 50, image: '', active: true },
  { id: 6, name: 'Corsair RM1000x', category: 'PSU', price: 4990000, stock: 18, image: '', active: false },
  { id: 7, name: 'NZXT H7 Flow', category: 'Case', price: 3590000, stock: 22, image: '', active: true },
  { id: 8, name: 'ASUS TUF Gaming Laptop F15', category: 'Laptop', price: 25990000, stock: 10, image: '', active: true },
];
...
export function ManagerProductsPage() {
  ...
  const [form, setForm] = useState({ name: '', category: 'CPU', price: '', stock: '', image: '', description: '' });
  ...
  const openCreate = () => {
    setForm({ name: '', category: 'CPU', price: '', stock: '', image: '', description: '' });
    setEditProduct(null);
    setModal('create');
  };

  const openEdit = (p: Product) => {
    setForm({ name: p.name, category: p.category, price: String(p.price), stock: String(p.stock), image: p.image, description: '' });
    setEditProduct(p);
    setModal('edit');
  };
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          ...
          <p style={{ fontSize: '14px', color: '#9ca3af', marginTop: '4px' }}>
            {products.length} sản phẩm · {products.filter(p => p.active).length} đang hiện · {products.filter(p => !p.active).length} đang ẩn
          </p>
        </div>
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' }}>
        ...
        >
          ...
          <option value="active" style={{ background: '#0f0a24', color: '#fff' }}>Đang hiện</option>
          <option value="inactive" style={{ background: '#0f0a24', color: '#fff' }}>Đang ẩn</option>
        </select>
      </div>
      ...
      <div style={{
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {filtered.map((p, i) => (
                <tr key={p.id} style={{ borderBottom: '1px solid rgba(245,158,11,0.05)', opacity: p.active ? 1 : 0.5 }}>
                  ...
                  <td style={{ padding: '14px 20px', fontSize: '14px', color: p.stock < 10 ? '#f59e0b' : '#d1d5db' }}>
                    {p.stock}
                    {p.stock < 10 && <span style={{ color: '#ef4444', fontSize: '12px', marginLeft: '6px' }}>Sắp hết</span>}
                  </td>
                  <td style={{ padding: '14px 20px' }}>
                    <span style={{
                    ...
                    }}>{p.active ? 'Đang hiện' : 'Đang ẩn'}</span>
                  </td>
                  <td style={{ padding: '14px 20px' }}>
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <button onClick={() => toggleActive(p.id)} title={p.active ? 'Ẩn' : 'Hiện'} style={{
                        padding: '8px', borderRadius: '8px', border: 'none', cursor: 'pointer',
                        background: 'rgba(245,158,11,0.15)', color: '#f59e0b', display: 'flex',
                      }}>
                        {p.active ? <EyeOff style={{ width: 16, height: 16 }} /> : <Eye style={{ width: 16, height: 16 }} />}
                      </button>
